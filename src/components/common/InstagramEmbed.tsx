        ...
        <div className={`bg-white rounded-lg shadow-md p-4 ${className}`}>
          ...
          <div className="mt-4 text-center">
            <a
              ...
              className="inline-block px-4 py-2 rounded-md bg-brand-500 text-white hover:bg-brand-600 transition"
            >
              View on Instagram
            </a>
          </div>
        ...
        <div className="mt-4 flex items-center justify-center gap-2">
          {images.map((_, i) => (
            <button
              key={i}
              onClick={() => setIndex(i)}
              className={`w-2 h-2 rounded-full ${i === index ? 'bg-brand-500' : 'bg-gray-300'}`}
