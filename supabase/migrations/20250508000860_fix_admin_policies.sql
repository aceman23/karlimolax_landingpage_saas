/*
  ...
  1. Updates
    - Drop existing admin policies that incorrectly check role
    - Create new policies that properly check for admin role in profiles table
...
DROP POLICY IF EXISTS "Admins can delete all drivers" ON drivers;
...
-- Create new admin policies that properly check role in profiles table
